    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "python -m pip install 'langchain-postgres>=0.0.12,<0.1'"
        ) from error
...
def add_documents_to_vector_store(vector_store, documents: list[Document]) -> None:
    ...
    ids = [
        f"{Path(str(document.metadata.get('source', 'unknown'))).as_posix()}:{document.metadata['chunk_index']}"
...
class LangChainRagPipeline:
    def __init__(self, retriever: VectorStoreRetriever, llm: OpenRouterClient) -> None:
    ...
    def ask(self, question: str) -> LangChainRagAnswer:
        ...
        return LangChainRagAnswer(answer=answer, sources=documents)
    ...
    def _format_documents(documents: list[Document]) -> str:
        ...
        return "\n\n".join(
            "[chunk {chunk_index} | source: {source}]\n{content}".format(
